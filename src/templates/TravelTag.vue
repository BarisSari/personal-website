<template>
  <div class="travel-tag-page">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <h1>Travel posts tagged with "{{ tagName }}"</h1>
          <div class="row">
            <div
            ...
            >
              <PostCard :post="post" type="travels" />
            </div>
          </div>
          <div v-if="taggedPosts.length === 0" class="no-posts">
            <p>No travel posts found with this tag.</p>
          </div>
        </div>
      </div>
    </div>
...
const taggedPosts = computed(() => {
  return getPostsByTag(tagName.value).filter(post => post.type === 'travels')
})
</script>
